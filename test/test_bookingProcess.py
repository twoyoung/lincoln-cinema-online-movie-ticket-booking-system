...
from app.models import Booking, Payment, Screening, CinemaHallSeat
...
def mock_seats(mocker):
    ...
    mock.query.filter.return_value.all.return_value = [MagicMock(CinemaHallSeat) for _ in range(5)]  # Mock 5 seat objects
    return mock
...
def test_process_booking(test_client, movie_controller, mock_screening, mock_seats, mock_booking, mock_payment_processor):
    user_id = 1
    screening_id = 1
    ...
    mock_seats.query.filter.assert_called_with(CinemaHallSeat.id.in_(selected_seat_ids))
